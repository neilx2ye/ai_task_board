...
AI Task Board 是面向个人和小团队的 AI 会话任务控制台。ChatGPT、Claude、Codex、Gemini 或自定义 Agent 在 CLI / APP 中建立上下文并完成推理与工具调用，通过 REST API 或 MCP 注册会话、同步当前工作、读取本会话的预留任务、拆分任务并回传进度；网页端负责向指定存活会话预留任务、查看状态和回复问题。

项目是一个 Next.js 单体应用，正式数据存储只依赖 **Supabase Hosted**：Auth、PostgreSQL/RPC、Realtime、RLS 与私有 Storage。不包含 SQLite、Docker Compose、自托管 Supabase、消息队列或内置模型调用。
...
父任务按后代状态聚合：`waiting_user` 优先，其次 `running/claimed`、`failed`、`ready`、`blocked`；直接有效子任务全部完成后父任务完成。结构化进度按已完成叶子数计算，AI 上报百分比仅作估计展示。所有命令在服务端/RPC 修改状态，并以只追加的方式写入 `task_events`；受控的 Workspace 物理清理仍会通过外键级联删除其历史数据。

## 当前限制

- MVP 面向个人或小团队，没有组织计费、复杂角色、自定义工作流或 DAG 可视化编辑器。
- 不内置任何模型、Agent 循环或工具执行环境；AI 必须在外部会话中运行。
- Realtime 只用于界面失效和重拉；没有自建 SSE/WebSocket。客户端维护最新 `TaskEvent` ID，断线重订阅后会按游标补拉遗漏事件并全量重拉，以 30 秒轮询作为低频兜底。
- 没有定时 Worker；原目标会话可原子恢复自己的过期租约，用户也可显式释放后改派。仅打开页面不会修改租约，卡片可能一直显示旧接收信息，直到下一次接收/释放命令。
...
- `heartbeat_ai_session` 维护空闲会话存活状态，`heartbeat_claim` 同时续期执行中任务；Web 只允许向两分钟内有心跳的会话预留新任务。
