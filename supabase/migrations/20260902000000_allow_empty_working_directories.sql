-- Web 期望清单此前不接受空数组：desired 为 null 表示“沿用本机启动配置”，
-- 非 null 则必须包含 1 到 100 个目录。删除设备唯一托管目录后“剩余清单”为
-- 空，无法表达“接管但暂不管理任何目录”，删除逻辑只能回退到 null，设备随后
-- 继续按启动配置上报该路径，刚删除的项目随之复活。这里把空数组放宽为合法
-- 状态（与 null 明确区分），并允许设备同步上报空目录清单，让 Board 把该
-- 运行时已有的目录行与 Session 一并置为不活跃。
...
begin
  if p_directories is null
     ...
     or jsonb_array_length(p_directories) > 100 then
    return false;
  end if;
...
comment on function public._bridge_working_directories_are_valid(jsonb) is
  'Web 目录清单形状校验；空数组表示 Web 接管但暂不管理任何目录，null 才表示沿用本机启动配置。';
...
begin
  ...
  -- 空数组合法：表示该运行时当前不管理任何目录，下面的清单更新会把既有
  -- 目录行全部置为不活跃。
