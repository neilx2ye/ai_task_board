...
import { resolveRemoteConfiguration } from "../../packages/claude-code-bridge/src/bridge";
...
describe("Claude Code Bridge remote working directories", () => {
  let temporaryDirectory: string | null = null;

  function baseDirectory(): string {
    temporaryDirectory = mkdtempSync(path.join(tmpdir(), "atb-claude-dirs-"));
...
describe("Claude Code Bridge session sync device identity", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
    vi.unstubAllEnvs();
  });
  ...
  it("sends flat device_id and device_label fields with the Claude capability version", async () => {
    ...
    expect(body.bridge_version).toBe("1.7.0-claude.1");
    expect(CLAUDE_BRIDGE_CAPABILITY_VERSION).toBe("1.7.0-claude.1");
  ...
  it("loads the persisted device identity by default", async () => {
    const configHome = mkdtempSync(path.join(tmpdir(), "atb-claude-device-"));
    try {
