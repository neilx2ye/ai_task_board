    ...
    while (!this.stopping) {
      ...
      if (this.inventoryReady) {
        try {
          ...
          process.stderr.write(
            `处理 Kimi 文件浏览指令失败：${errorMessage(error)}\n`,
          );
        }
      }
      try {
  ...
  private configurationStatus(releaseRuntime = false): Record<string, unknown> {
    ...
    return {
      ...
      constraints: {
        ...
        // Thread count and turn concurrency are owned by the Web setting; the
        // fixed values mirror the Board-side schema caps for compatibility.
  ...
  private async processFileCommands(): Promise<void> {
    for (let processed = 0; processed < 10 && !this.stopping; processed += 1) {
      ...
      try {
        ...
        process.stderr.write(
          `Web Kimi 文件指令 ${command.action} 失败：${message}\n`,
        );
      }
    }
  }
