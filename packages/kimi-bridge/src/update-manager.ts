...
const RUNTIME_DIST_SUBDIRECTORY = path.join("dist", "kimi-runtime");
...
// Bridge, so this runtime's own suffixed constant (e.g. 1.4.0-kimi.1) already
...
export async function maybeApplyDesiredBridgeUpdate(
  ...
  if (!environment.INVOCATION_ID?.trim()) {
    if (noticedSkipTarget !== desired) {
      ...
      log(
        `看板请求将 Kimi Bridge 升级到 ${desired}，但当前进程不在 systemd 下运行` +
          "；已跳过，请手动升级",
      );
    }
    return null;
  ...
  if (
    ...
    if (noticedSkipTarget !== desired) {
      noticedSkipTarget = desired;
      log(
        `看板请求将 Bridge 升级到 ${desired}，但统一设备 Bridge 的升级由 ${environment.AI_TASK_BOARD_BRIDGE_UPDATE_LEADER || "leader"} 运行时执行` +
          "；本运行时随服务一起重启",
      );
    }
    return null;
  ...
  try {
    ...
    const message = redactText(
      `升级到 Kimi Bridge ${desired} 失败：${errorMessage(error)}`,
      2_000,
    );
    log(message);
    return message;
...
async function applyDesiredBridgeUpdate(
  ...
  const staging = await mkdtemp(
    path.join(tmpdir(), "ai-task-board-kimi-bridge-update-"),
  );
  try {
  ...
  log(
    `Kimi Bridge ${desired} 已安装并写入 systemd unit；本进程即将退出，将在约 5 秒后由 systemd 重启到新版本`,
  );
