...
export const CLAUDE_BRIDGE_CAPABILITY_VERSION = "1.7.0-claude.1";
...
function idempotencyKey(operation: string): string {
  return `claude-bridge/${operation}/${randomUUID()}`;
}
  ...
  async syncSessions(
    ...
    const response = await this.request<{ sessions: BoardSession[] }>(
      "/api/ai/sessions/sync",
      {
        ...
        body: {
          bridge_version: CLAUDE_BRIDGE_CAPABILITY_VERSION,
          platform: "claude",
  ...
  reportProgress(
    ...
    return this.request("/api/ai/tasks/report-progress", {
      ...
      body: {
        ...
        progress_note: "Claude Code ACP 已接收任务，正在执行",
  ...
  reportAssistantMessage(
    ...
    return this.request("/api/ai/sessions/activity", {
      ...
      body: {
        task_id: task.id,
        claim_token: task.claim_token,
        external_ref: `claude:${externalThreadId}:${task.id}:assistant`,
...
export function actionableBoardError(error: unknown): Error {
  ...
  return new Error(
    `看板拒绝 Claude Bridge 请求（HTTP ${status ?? "unknown"}）：${detail}`,
    { cause: error },
  );
}
