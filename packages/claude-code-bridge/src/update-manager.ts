...
const RUNTIME_DIST_SUBDIRECTORY = path.join("dist", "claude-runtime");
...
// Bridge, so this runtime's own suffixed constant (e.g. 1.4.0-claude.1) already
...
export async function maybeApplyDesiredBridgeUpdate(
  ...
  if (!environment.INVOCATION_ID?.trim()) {
    if (noticedSkipTarget !== desired) {
      ...
      log(
        `看板请求将 Claude Bridge 升级到 ${desired}，但当前进程不在 systemd 下运行` +
          "；已跳过，请手动升级",
      );
    }
    return null;
  ...
  try {
    ...
    const message = redactText(
      `升级到 Claude Bridge ${desired} 失败：${errorMessage(error)}`,
      2_000,
    );
    log(message);
    return message;
...
async function applyDesiredBridgeUpdate(
  ...
  const staging = await mkdtemp(
    path.join(tmpdir(), "ai-task-board-claude-bridge-update-"),
  );
  try {
    const registry = (
  ...
  log(
    `Claude Bridge ${desired} 已安装并写入 systemd unit；本进程即将退出，将在约 5 秒后由 systemd 重启到新版本`,
  );
