...
Codex Bridge 0.2 是运行在 Codex 设备上的常驻 companion。一个 Bridge 进程对应一台设备上的一个 AI Connection；它通过 stdio 启动本机 `codex app-server`，自动发现多个未归档的顶层 Codex thread，并为每个 thread 在 AI Task Board 中同步一个独立 Session。
...
1. Board 已先升级到 0.2 所需的 Session 清单同步 API 和数据库 migration。Bridge 不会把同步 API 的 `404` 回退为旧版逐 Session 注册，版本不匹配会直接启动失败。
...
4. 本机已有至少一个 Codex thread。0.2 MVP 会发现、恢复和管理既有 thread，但尚不从网页创建新 thread。
...
ExecStart=/absolute/path/to/npx --yes ai-task-board-codex-bridge@0.2.0
Restart=on-failure
RestartSec=5
KillSignal=SIGTERM
TimeoutStopSec=30
...
环境文件至少包含 `AI_TASK_BOARD_URL` 和 `AI_TASK_BOARD_CONNECTION_TOKEN`。建议同时显式设置 `CODEX_BINARY`、`CODEX_WORKING_DIRECTORY`、`CODEX_THREAD_SCOPE=cwd`、`CODEX_MAX_THREADS`、`CODEX_MAX_CONCURRENT_TURNS`；也可设置 `CODEX_THREAD_ID` 精确限定。将文件权限设为 `0600`，再执行：
...
2. Bridge 分页读取当前用户最近的未归档 Codex thread，忽略子 Agent thread，并应用默认的 cwd 精确范围、显式 `all` 范围或可选的 `CODEX_THREAD_ID` 精确过滤器，再应用 `CODEX_MAX_THREADS` 上限。
3. Bridge 将完整清单同步到 Board。每个本地 thread 的稳定 ID 都映射为一个 Board Session；以后发现的新 thread 会在下一次清单同步加入，离开清单的 worker 会停止。
4. 每个 Session 建立自己的认证 SSE 唤醒流并保持心跳。SSE 只传固定的 `ready` / `wake` 提示，真正的任务仍通过 REST 原子领取；断线时自适应轮询兜底。
5. 每个 thread 一次只执行一张 Task。设备级 `CODEX_MAX_CONCURRENT_TURNS` 限制不同 thread 同时运行的 turn 数，多余工作继续排队。
6. Bridge 调用 `thread/resume` 和 `turn/start`，再消费 App Server 的 JSONL 通知。AI 文本、思考摘要和命令输出 delta 会按约 500 毫秒或 8 KiB 聚合后上传，完成事件也会持久化。
7. `turn/completed` 后，最后一条 AI 消息用于完成 Task；错误会把 Task 标记为失败。Bridge 随后继续处理对应 Session 队列。

网页在某个 thread 正忙时发送的新消息会创建下一张排队 Task。0.2 MVP 不会把它可靠地 steer 到当前 turn。
...
活动使用稳定的 thread/turn/item/chunk 外部引用去重。HTTP 5xx、限流或临时网络错误会退避重试。常见 Token、Authorization、API key、password 和 secret 形式会做尽力脱敏，过长内容会被截断；这不是完整的数据防泄漏系统，仍应限制 Codex 可读 Secret，并评估把本机输出同步到共享 Board 的边界。Inventory 必然上传 thread ID、绝对 `working_directory` 和模型标签给该 Workspace 的成员；只有显式开启 `CODEX_BRIDGE_INCLUDE_THREAD_TITLES=true` 时才额外把 thread 标题/首条 prompt 预览用于 Session 名称。
...
- **不创建 thread，也不补录完整旧历史。** 网页目前只能选择已发现的本地顶层 thread。既有历史仍可供 Codex 上下文使用，但 Board 只展示原先保存的内容和 Bridge 0.2 之后收到的新活动。
...
- **不会自动进入 `waiting_user`。** 普通 AI 回复中的问题不会自动改变 Task 状态，用户回复会成为下一张 Task。
...
- 同步 API 返回 `404`：先升级 Board 数据库 migration 与 API；0.2 Bridge 不会回退到旧版注册接口。
